...
4. **Vaihe 4: Frontend & Visualisointi** — VALMIS (`frontend/index.html`)
   ...
   * **Puskurin/top-korostuksen paksunnus on sama pikselisäde (`BUFFER_VISUAL_DILATION_PX`/`LOW_SCORE_EMPHASIS_PX`) jokaisella tasolla**, ei metrisäde — muuten ohut rantaviiva-indikaattori häviäisi karkeammalla resoluutiolla resize-pehmennyksessä. Tästä syystä `near`/`mid`/`overview`-tasot lasketaan raa'asta pistemäärästä/puskurimaskista uudelleen (downsample + dilataatio), ei suoraan valmiin `detail`-kuvan pienentämällä.
...
5. **Vaihe 5: Täysin staattinen julkaisu (GitHub Pages)** — VALMIS (`build_static.py`)
   ...
   * `python3 build_static.py` (ajetaan projektin juuresta): käy läpi `backend/pipeline.py`:n kautta kaikki tiilet ja kaikki neljä resoluutiotasoa (ks. kohta 4:n moniresoluutio-löydös), kirjoittaa `docs/cache/{tile}.png` (+ `_base.png`, `_top.png`, ja kaikista kolme `_near`/`_mid`/`_overview`-versiota) ja `docs/tiles.json` (tiilirajat + top-persentiilin kynnysarvo), ja generoi `docs/index.html`:n `frontend/index.html`:sta korvaamalla `/api/...`-polut suhteellisilla staattisilla poluilla (`URL_REPLACEMENTS`-taulukko skriptissä).
